...
(function(CKEDITOR) {
  CKEDITOR.plugins.add("axe", {
    ...
    init: function(editor) {
      ...
      if (typeof editor.config.axe !== "undefined") {
        ...
        if (typeof editor.config.axe.run !== "undefined") {
          ...
          let rules = plugin.axe.run.rules;
    ...
    axe: {
       ...
       */
      callback: function(error, results, editorName) {
        ...
        if (
          ...
          let panel = new CKEDITOR.ui.balloonPanel(editor, {
            title: title,
            content: content
          });

          // Balloon panel demo.
          let target = new CKEDITOR.dom.element(
            editor.document.$.getElementById("test_image")
          );
          panel.attach(target);
        }
        return true;
