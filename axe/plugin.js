  ...
  CKEDITOR.plugins.add("axe", {
    ...
    onLoad: function onLoad() {
      ...
      axe.callback = function (error, results, editorName) {
        ...
        console.log(results); // Validate editor instance.
        ...
        var editor = CKEDITOR.instances[editorName]; // In case if we have at least one violation.
      ...
      axe.override = function (settings) {
        ...
        if (typeof this.path !== "string") {
          ...
          var scripts = document.querySelectorAll( // @todo: Check compatibility of the attribute selectors.
          "script[src$='/axe.js'],script[src$='/axe.min.js']"); // Set to static variable.

          if (scripts.length) {
            // @todo: Detect usage of min.js and dev version.
            this.path = scripts[0].src;
          } else {
            console.error( // @todo: Multilanguage.
    ...
    init: function init(editor) {
      ...
      var plugin = this; // @todo: Multilanguage.

      var lang = editor.lang.axe; // Override default plugin settings.

      plugin.axe.override(editor.config.axe); // Add extra scripts when editor is ready.
